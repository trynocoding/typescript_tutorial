...
```typescript
// 推断 Promise 的内部类型
// ⚠️ 注意：这是教学用的简化版实现，不处理嵌套 Promise（如 Promise<Promise<string>>）
// TypeScript 内置的 Awaited<T>（4.5+）会递归解包，应优先使用内置版本
type Awaited<T> = T extends Promise<infer U> ? U : T

type A = Awaited<Promise<string>>  // string
type B = Awaited<number>           // number
// 内置版本额外处理：Awaited<Promise<Promise<string>>> = string（递归解包）
...
const id: UserId = 1
getOrder(id)  // ⚠️ TypeScript 不会报错！
               // UserId 和 OrderId 都是 number 的别名，结构上完全兼容
               // 这正是我们需要「品牌类型」的原因
```
...
// 带类型注解（不用 satisfies）：
const WITHOUT_SATISFIES: Record<string, string> = {
    motion: "move",
    change: "modify"
}
// WITHOUT_SATISFIES.motion 的类型是 string
// 因为类型注解会拓宽推断，将字面量类型択宽为 string

// 用 as const 但不用 satisfies：
const WITH_CONST_ONLY = {
    motion: "move",
    change: "modify"
} as const
// WITH_CONST_ONLY.motion 的类型是 "move"（字面量）
// as const 是字面量类型的来源，而不是 satisfies

// satisfies 的优势：在保留字面量类型的同时，验证结构符合指定类型
const WITH_SATISFIES = {
    motion: "move",
    change: "modify",
    //invalid: 123  // ⚠️ satisfies 会报错：number 不兼容 string
} as const satisfies Record<string, string>
// WITH_SATISFIES.motion 的类型是 "move"（字面量）—— 来自 as const
// satisfies 提供额外的结构验证，但不改变推断宽度
```
